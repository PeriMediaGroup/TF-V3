...
export default function MediaPickerSheet({
  ...
  const renderItem = ({ item, index }) => {
    if (index === 0) {
      return (
        <TouchableOpacity style={[styles.tile, { backgroundColor: theme?.card }]} onPress={cameraAction}>
          <Ionicons
    ...
    return (
      <TouchableOpacity style={styles.tile} onPress={() => toggle(uri)}>
        ...
        {picked && (
          <View style={[styles.check, { borderColor: theme?.background, backgroundColor: theme?.primary }]}>
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={[styles.wrap, { backgroundColor: theme?.background }]}> 
        <View style={[styles.header, { borderBottomColor: theme?.border }]}> 
          <Text style={[styles.title, { color: theme?.text }]}>{isVideo ? 'Select Video' : 'Select Photos'}</Text>
        ...
        <View style={[styles.footer, { borderTopColor: theme?.border, backgroundColor: theme?.surface }]}> 
          <Text style={{ color: theme?.text }}>
            {isVideo ? (selected.length ? '1 selected' : 'Select one video') : `${selected.length}/${maxCount} selected`}
          </Text>
          <TouchableOpacity onPress={confirm} disabled={!selected.length} style={[styles.addBtn, { backgroundColor: selected.length ? theme?.primary : theme?.cardSoft }]}> 
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 12, paddingVertical: 12, borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  title: { fontSize: 18, fontWeight: '700' },
  tile: { width: '31%', aspectRatio: 1, margin: '1%', borderRadius: 8, overflow: 'hidden', alignItems: 'center', justifyContent: 'center' },
  thumb: { width: '100%', height: '100%' },
  check: { position: 'absolute', top: 6, right: 6, width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  footer: { padding: 12, borderTopWidth: StyleSheet.hairlineWidth, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
